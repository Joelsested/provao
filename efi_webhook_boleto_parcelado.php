...
function atualizarStatusPagamentoBoleto($pdo, $chargeId, $status, $dataPagamento = null)
{
    try {
        ...
        if (!empty($dataPagamento)) {
            $sql .= ", data_pagamento = :data_pagamento";
            $params[':data_pagamento'] = $dataPagamento;
        }
        $sql .= " WHERE charge_id = :charge_id";
...
try {
    ...
    if ($currentStatus === 'paid') {
        ...
        try {
            ...
            $dataPagamento = $lastItem['status']['update_at']
                ?? $lastItem['status']['updated_at']
                ?? $lastItem['status']['created_at']
                ?? $receivedAt;
