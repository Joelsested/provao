...
function excluir(id){
    var idStr = '';
    function buscarDataId(el) {
        var current = el;
        while (current && current !== document) {
            if (current.getAttribute) {
                var val = current.getAttribute('data-id');
                if (val) {
    ...
    function buscarDataIdPorSelecao() {
        var $openMenu = $('.dropdown.open .dropdown-menu a[data-id]').first();
        if ($openMenu.length) {
            return String($openMenu.data('id')).trim();
        }
        var $selected = $('#tabela').find('a[data-id]').filter(function () {
            return $(this).closest('ul.dropdown-menu:visible').length > 0;
        }).first();
        if ($selected.length) {
            return String($selected.data('id')).trim();
        }
        return '';
    }
    var el = null;
    if (id && typeof id === 'object' && id.target) {
        el = id.currentTarget || id.target;
    } else if (id && typeof id === 'object' && id.getAttribute) {
        el = id;
    }
    if (el) {
        idStr = buscarDataId(el);
    }
    if (!idStr) {
        idStr = (id === undefined || id === null) ? '' : String(id).trim();
    }
    if (!idStr && id && typeof id === 'object' && id.target) {
        idStr = buscarDataId(id.target);
    }
    if (!idStr) {
        idStr = buscarDataIdPorSelecao();
    }

    if (idStr === '') {
        var $row = $(el || id).closest('tr');
        var rowId = $row.data('id');
        if (rowId) {
            idStr = String(rowId).trim();
        }
    }
    if (idStr === '') {
        var $rowActive = $('#tabela').find('tr.active, tr.selected').first();
        if ($rowActive.length && $rowActive.data('id')) {
            idStr = String($rowActive.data('id')).trim();
        }
    }
    if (idStr === '') {
        $('#mensagem-excluir').addClass('text-danger')
        $('#mensagem-excluir').text('ID invalido.')
        return;
    }
    $.ajax({
        url: 'paginas/' + pag + "/excluir.php",
        method: 'POST',
        data: { id: idStr, id_matricula: idStr, csrf_token: getCsrfToken() },
        dataType: "text",
