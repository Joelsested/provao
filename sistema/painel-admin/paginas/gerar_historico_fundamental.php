                    ...
                    if (is_dir($dir)) {
                        ...
                        if (!empty($arquivos)) {
                            ...
                            foreach ($arquivos as $arquivo) {
                                ...
                                $caminhoRel = "historicos/{$id}/fundamental/" . $nomeArquivo;
                                $caminhoUrl = $baseUrl . $caminhoRel;

                                echo '<tr>';
                                echo "<td class='px-6 py-4 whitespace-nowrap'>{$nomeArquivo}</td>";
                                echo "<td class='px-6 py-4 text-center'>
                                <button onclick=\"abrirHistorico('{$caminhoUrl}')\" class='text-blue-500 hover:text-blue-700'>
                                    🔍
                                </button>
                                 <button onclick=\"downloadHistorico('{$caminhoUrl}')\" class='text-blue-500 hover:text-blue-700'>
                                    ⬇️
                                </button>
                                <button onclick=\"apagarHistorico('{$caminhoRel}')\" class='text-red-500 hover:text-red-700 ml-3'>
                                    🗑️
                                </button>
                            </td>";
                                echo '</tr>';
        ...
        function abrirFormularioNotas() {
           ...
           <td>
            ${!valorCampo1
                        ...
                        : `<input type="date" value="${notasExistentes[nomeMateria]['data_certificado'].split('-').reverse().join('-')}"  id="${materia.replace(/\s+/g, '_')}_date"  name="${materia.replace(/\s+/g, '_')}_date" value="${dataFormatada}">`
                    }
             </td>
            ...
            <td><input type="number" class="grade-input" id="${materia.replace(/\s+/g, '_')}_1" min="0" max="10" step="0.1" placeholder="0.0" value="${valorCampo}"></td>
            <td><input type="number" class="grade-input" id="${materia.replace(/\s+/g, '_')}_2" min="0" max="10" step="0.1" placeholder="0.0" value="${valorCampo}"></td>
            <td><input type="number" class="grade-input" id="${materia.replace(/\s+/g, '_')}_3" min="0" max="10" step="0.1" placeholder="0.0" value="${valorCampo}"></td>
            <td><input type="number" class="grade-input" id="${materia.replace(/\s+/g, '_')}_3" min="0" max="10" step="0.1" placeholder="0.0" value="${valorCampo}"></td>
            <td>
            ${!valorCampo
                        ? `<input type="date" id="${materia.replace(/\s+/g, '_')}_date" name="${materia.replace(/\s+/g, '_')}_date">`
                    ...
                    attributes: {
                        innerHTML: `
                            <div class="form-group">
                                ...
                                <input type="text" id="escola" class="swal-content__input" placeholder="Ex: Centro de Estudos SESTED" value="SESTED">
                            </div>
                            <div class="form-group">
                                <label>Município/UF:</label>
                                <input type="text" id="municipio" class="swal-content__input" placeholder="Ex: Buritis" value="">
                            </div>
                             <div class="form-group">
                                <label>UF:</label>
                                <input type="text" id="uf" class="swal-content__input" placeholder="Ex: RO" value="RO">
                            </div>

                             <div class="form-group">
                                <label>CERTIDÃO NASCIMENTO:</label>
                                <input type="text" id="certidao_nascimento" class="swal-content__input" placeholder="Ex: 123456789">
                            </div>
                            
                                <div class="form-group">
    <label>DATA HISTÓRICO:</label>
    <input type="date" id="data_historico" class="swal-content__input" value="${new Date().toISOString().split('T')[0]}" onclick="this.showPicker()" onfocus="this.showPicker()">
</div>

                        <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center;">
                            <div class="form-group" style="width: 48%;">
                                <label>Ano de Conclusão:</label>
                                <input type="number" id="anoConclusao" class="swal-content__input" placeholder="Ex: 2024" min="1990" max="2030" value="2025">
                            </div>
                            <div class="form-group" style="width: 48%;">
