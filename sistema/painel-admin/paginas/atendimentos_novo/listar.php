...
$placeholders = implode(',', array_fill(0, count($usuarioRelacionados), '?'));
$query = $pdo->prepare("SELECT * FROM alunos WHERE {$exprResponsavel} IN ($placeholders) ORDER BY id desc");
$query->execute($usuarioRelacionados);
...
HTML;
	...
	for ($i = 0; $i < $total_reg; $i++) {
		...
		$atendente_id = (int) ($res_query_alunos2[$i]['usuario'] ?? 0);

		$responsavel_id = (int) ($res_query_alunos2[$i]['responsavel_id'] ?? 0);
		if ($responsavel_id <= 0) {
			$responsavel_id = $atendente_id;
		}
	...
	$(document).ready(function () {
		function iniciarTabelaAtendimentosNovo(tentativas) {
			...
			if (!$.fn.DataTable) {
				if (tentativas > 0) {
					setTimeout(function () {
						iniciarTabelaAtendimentosNovo(tentativas - 1);
					}, 150);
				}
				return;
			}
