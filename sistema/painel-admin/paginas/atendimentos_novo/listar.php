...
if (@$_SESSION['nivel'] == 'Professor' || @$_SESSION['nivel'] == 'Tutor' || @$_SESSION['nivel'] == 'Parceiro' || @$_SESSION['nivel'] == 'Vendedor') {
	...
	$query_alunos1 = $pdo->prepare("SELECT * FROM alunos WHERE usuario = :usuario");
	$query_alunos1->execute([':usuario' => $id_user]);
	$res_query_alunos2 = $query_alunos1->fetchAll(PDO::FETCH_ASSOC);

	$total_reg = @count($res_query_alunos2);

} else {

	$query = $pdo->prepare("SELECT * FROM alunos WHERE usuario = :usuario");
	$query->execute([':usuario' => $id_user]);
	$res_query_alunos2 = $query->fetchAll(PDO::FETCH_ASSOC);
...
HTML;
	...
	for ($i = 0; $i < $total_reg; $i++) {
		...
		foreach ($res_query_alunos2[$i] as $key => $value) {

		}

		$id = $res_query_alunos2[$i]['id'];	

		$nome = $res_query_alunos2[$i]['nome'];

		$cpf = $res_query_alunos2[$i]['cpf'];

		$email = $res_query_alunos2[$i]['email'];

		$rg = $res_query_alunos2[$i]['rg'];
		$orgao_expedidor = $res_query_alunos2[$i]['orgao_expedidor'] ?? '';

		$expedicao = $res_query_alunos2[$i]['expedicao'];

		$telefone = $res_query_alunos2[$i]['telefone'];

		$cep = $res_query_alunos2[$i]['cep'];

		$endereco = $res_query_alunos2[$i]['endereco'];

		$cidade = $res_query_alunos2[$i]['cidade'];

		$estado = $res_query_alunos2[$i]['estado'];

		$sexo = $res_query_alunos2[$i]['sexo'];

		$nascimento = $res_query_alunos2[$i]['nascimento'];

		$mae = $res_query_alunos2[$i]['mae'];

		$pai = $res_query_alunos2[$i]['pai'];

		$naturalidade = $res_query_alunos2[$i]['naturalidade'];

		$professor4 = $res_query_alunos2[$i]['usuario'];

		$foto = $res_query_alunos2[$i]['foto'];

		$data = $res_query_alunos2[$i]['data'];



		$ativo = $res_query_alunos2[$i]['ativo'];

		$arquivo = $res_query_alunos2[$i]['arquivo'];
...
<a href="index.php?pagina=arquivos_alunos&usuario={$email}"   title="Arquivos do aluno" ><big><i class="fa fa-file-pdf-o text-success"></i></big></a>
		...
		<big><a href="#" onclick="mostrar( '{$nome}','{$cpf}','{$email}','{$rg}','{$orgao_expedidor}','{$expedicao}','{$telefone}','{$cep}','{$endereco}','{$cidade}','{$estado}','{$sexo}','{$nascimento}','{$mae}','{$pai}','{$naturalidade}', '{$foto}', '{$dataF}', '{$ativo}', '{$senha_usuario}','{$arquivo}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function mostrar(nome, cpf, email, rg, orgao_expedidor, expedicao, telefone, cep, endereco, cidade, estado, sexo, nascimento, mae, pai, naturalidade, foto, data, ativo, senha, arquivo) {
