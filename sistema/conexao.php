...
if ($isPanel && empty($_SESSION['nivel']) && $method !== 'GET' && $method !== 'HEAD' && $method !== 'OPTIONS') {
	$logPath = __DIR__ . '/auth_debug.log';
	$logLine = date('Y-m-d H:i:s') . ' script=' . ($scriptName ?: '') . ' method=' . $method;
	$logLine .= ' session_id=' . session_id() . ' cookie=' . ($_SERVER['HTTP_COOKIE'] ?? '');
	$logLine .= ' host=' . ($_SERVER['HTTP_HOST'] ?? '') . PHP_EOL;
