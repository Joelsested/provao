...
$query_m = $pdo->prepare("SELECT * FROM perguntas_quest where curso = :curso");
$query_m->execute([':curso' => $curso]);
$res_m = $query_m->fetchAll(PDO::FETCH_ASSOC);
...
if($questionario_config != 'Sim' or $curso_quest == 0){
	...
	if($status_mat != 'Finalizado' and $total_aulas == $aulas_conc){
		$query_m = $pdo->prepare("UPDATE matriculas SET status = 'Finalizado', data_conclusao = curDate() where id = :id");
...
if($aula == 'proximo' and $sessao != 0){
	...
	$query = $pdo->prepare("SELECT * FROM aulas where curso = :curso and sequencia_aula = :sequencia");
	$query->execute([
		':curso' => $curso,
		':sequencia' => $proxima,
	]);
