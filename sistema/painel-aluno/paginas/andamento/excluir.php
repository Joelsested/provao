...
$stmtBusca = $pdo->prepare("SELECT id, id_curso, pacote, obs FROM {$tabela} WHERE id = :id AND aluno = :aluno LIMIT 1");
$stmtBusca->execute([
	':id' => $id,
	':aluno' => $aluno,
]);
...
try {
	...
	$stmtDelete = $pdo->prepare("DELETE FROM {$tabela} WHERE id = :id AND aluno = :aluno LIMIT 1");
	$stmtDelete->execute([
		':id' => $matricula['id'],
		':aluno' => $aluno,
	]);
	...
	if ($matricula['pacote'] === 'Sim') {
		...
		$stmtFilhos = $pdo->prepare("SELECT id FROM {$tabela} WHERE aluno = :aluno AND pacote <> 'Sim' AND id_pacote = :id_pacote AND obs = 'Pacote'");
		$stmtFilhos->execute([
			':aluno' => $aluno,
			':id_pacote' => $idPacote,
		]);
		...
		if (!empty($idsFilhos)) {
			...
			$pdo->prepare("DELETE FROM {$tabela} WHERE id IN ($placeholders) AND aluno = ?")->execute($idsFilhosComAluno);
		}
	}
