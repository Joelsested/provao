            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($boleto_transactions as $registro): ?>
                        <?php
                        $statusApi = '';
                        if (!empty($registro['charge_id'])) {
                            $statusApi = buscarStatusBoleto($boletoPayment, $registro['charge_id'], $status_cache);
                        }
                        $statusResumo = resumirStatusBoleto($statusApi);
                        if ($statusResumo === 'Nao Gerado') {
                            if ($registro['status'] === '' || $registro['status'] === 'peding') {
                                $statusResumo = 'Pendente';
                            } elseif ($registro['status'] === 'paid') {
                                $statusResumo = 'Pago';
                            } else {
                                $statusResumo = $registro['status'];
                            }
                        }
    ...
    <button
        ...
        Copiar Pix
                            ...
                            <td>
                                ...
                                <?php elseif ($statusResumo === 'Pago'): ?>
                                    <button
                                        onclick="window.open('<?php echo htmlspecialchars($registro['url_boleto'], ENT_QUOTES); ?>', '_blank')">
                                        <i class="fa fa-file-pdf-o" aria-hidden="true"></i>
                                        Ver Detalhes
                                    </button>
                                <?php elseif ($statusResumo !== 'Pago' && !empty($registro['charge_id'])): ?>
                                    <input type="date" class="form-control" style="display:inline-block; width:auto;"
                                        id="vencimento-boleto-<?php echo (int) $registro['id']; ?>"
                                        min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>"
                                        title="Nova data de vencimento">
                                    <button type="button"
                                        onclick="solicitarNovoVencimento('boleto', <?php echo (int) $registro['id']; ?>)"
                                        title="Atualizar vencimento">
                                        Atualizar
                                    </button>
                                <?php else: ?>
