...
async function escolherResponsavel() {
    try {
        ...
        const html = `
            ...
            <p>Escolha o responsável (vendedor, tutor, secretário ou tesoureiro) que confirma esta matrícula.</p>
...
async function atualizarResponsavelResumo() {
    ...
    try {
        const data = await obterResponsaveis();
        if (!data.success) {
            resumo.textContent = data.message || 'Não foi possível carregar o responsável.';
            return;
        }
        const current = data.current;
        const options = data.options || [];
        if (current && current.id && !responsavelSelecionado) {
            responsavelSelecionado = current.id;
            resumo.textContent = `${current.nome} (${current.nivel})`;
            return;
        }
        if (responsavelSelecionado) {
            const escolhaAtual = options.find((item) => item.id == responsavelSelecionado);
            if (escolhaAtual) {
                resumo.textContent = `Selecionado: ${escolhaAtual.nome} (${escolhaAtual.nivel})`;
                return;
            }
        }
        if (options.length > 0) {
            ...
            resumo.textContent = `Responsável sugerido: ${sugerido.nome} (${sugerido.nivel})`;
            return;
        }
		...
		$.ajax({
			...
			success: function (mensagem) {
				...
				if (partes[0].trim() == "Logado com Sucesso") {
					...
					if (partes[2]) {
						window.CSRF_TOKEN = partes.slice(2).join('-').trim();
					}
