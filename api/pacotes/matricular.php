...
if(@count($res) > 0){
	$result = json_encode(array('mensagem'=>'Aluno jǭ Matriculado no Pacote!', 'sucesso'=>false));
	echo $result;
	exit();
}else{
	...
	$stmtInsert = $pdo->prepare("INSERT INTO matriculas SET id_curso = :curso, aluno = :aluno, professor = :professor, valor = :valor, data = curDate(), status = :status, pacote = 'Sim', subtotal = :subtotal, aulas_concluidas = '1'");
